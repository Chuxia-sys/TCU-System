...
export const HEADER_HEIGHT = 72; // Height in pixels

export function UnifiedHeader() {
  const { data: session } = useSession();
  const { theme, setTheme } = useTheme();
  const { setViewMode, sidebarCollapsed, setSidebarCollapsed } = useAppStore();
  ...
  return (
    <header className="shrink-0 flex h-[72px] border-b border-border/5 bg-[#8b0000] dark:header-gradient z-40">
      ...
      >
        ...
        {/* Burger button - always visible in header */}
        <Button
          variant="ghost"
          size="icon"
        ...
        >
          <Menu className={sidebarCollapsed ? 'h-3.5 w-3.5' : 'h-4 w-4'} />
        </Button>
      </div>
      ...
      <div className="flex-1 flex items-center gap-2 sm:gap-4 px-4 lg:px-6 bg-[#8b0000] dark:header-gradient">
